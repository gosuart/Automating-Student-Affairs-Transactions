...
class StudentRepository @Inject constructor(
    ...
    suspend fun getStudentData(): Flow<Result<StudentProfile>> = flow {
        try {
            ...
            // محاولة جلب البيانات من الخادم الرئيسي أولاً
            try {
                ...
                // إذا فشل الخادم الرئيسي، جرب الخادم الفرعي
            }
            
            // محاولة جلب البيانات من الخادم الفرعي
            val profileResponse = apiService.getProfile(studentId)
            ...
            if (profileResponse.isSuccessful) {
                ...
                if (apiResponse?.success == true && apiResponse.data != null) {
                    ...
                    emit(Result.failure(Exception(apiResponse?.message ?: "خطأ في جلب البيانات الشخصية")))
                }
            } else {
                emit(Result.failure(Exception("خطأ في الاتصال بالخادم")))
            }
    ...
    suspend fun changePassword(
        ...
        try {
            ...
            if (response.isSuccessful) {
                val apiResponse = response.body()
                if (apiResponse?.success == true) {
                    emit(Result.success(apiResponse.message ?: "تم تغيير كلمة المرور بنجاح"))
                } else {
                    emit(Result.failure(Exception(apiResponse?.message ?: "فشل في تغيير كلمة المرور")))
                }
            } else {
                emit(Result.failure(Exception("خطأ في الاتصال بالخادم")))
            }
    ...
    suspend fun updateSessionTime() {
        preferencesManager.updateSessionTime()
    }
    
    // التحقق من صلاحية الجلسة
    suspend fun isSessionValid(): Boolean {
        return preferencesManager.isSessionValid()
    }
    ...
    suspend fun getProfile(): Flow<Result<StudentProfile>> = flow {
        try {
            ...
            val response = apiService.getProfile(studentId)
