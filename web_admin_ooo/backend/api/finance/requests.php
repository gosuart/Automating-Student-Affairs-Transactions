...
function getCardsData($db) {
    ...
    try {
        ...
        foreach ($rows as &$row) {
            ...
            // التحقق من أن هذه الخطوة مخصصة للمالية
...
function approveRequest($db) {
    ...
    $result = $updateStmt->execute([$notes, $trackingId]);
    ...
    if ($result) {
        ...
        if ($requestInfo) {
            ...
            if ($nextStep) {
                ...
                $message = 'تم تأكيد الدفع بنجاح وانتقل الطلب إلى خطوة: ' . $nextStep['step_name'];
            } else {
                // لا توجد خطوة تالية - الطلب مكتمل
                $updateRequestSql = "UPDATE requests SET status = 'completed' WHERE id = ?";
                $updateRequestStmt = $db->prepare($updateRequestSql);
                $updateRequestStmt->execute([$requestId]);
                
                $message = 'تم تأكيد الدفع بنجاح وتم إكمال جميع خطوات الطلب';
            }
        }
