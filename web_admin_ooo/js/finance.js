// متغيرات عامة لصفحة شؤون الطلاب
let requestsData = [];
let filteredRequestsData = [];
let selectedRequestId = null;
let currentUser = null;

// إعداد صفحة شؤون الطلاب عند تحميل الصفحة
document.addEventListener('DOMContentLoaded', function() {
    ...
    setupStudentAffairsPage();
});

// إعداد صفحة شؤون الطلاب
function setupStudentAffairsPage() {
    // عرض معلومات المستخدم
    document.getElementById('userName').textContent = currentUser.name || 'موظف الماليه';
...
async function loadRequests() {
    ...
    try {
        const response = await fetch('../backend/api/finance/requests.php?action=get_cards_data');
...
function displayRequestsTable(requests) {
    ...
    requests.forEach(request => {
        ...
        if (request.amount && request.amount > 0) {
            amount = `<strong style="color: #28a745;">${request.amount} ريال</strong>`;
        } else {
            amount = '<span class="text-muted">غير محدد</span>';
        }
        ...
        row.innerHTML = `
            ...
            <td class="actions-cell">
                <button class="btn btn-sm btn-info" onclick="viewRequestDetails(${request.tracking_id})" title="عرض التفاصيل">
                    <i class="fas fa-eye"></i>
                </button>
                <button class="btn btn-sm btn-success" onclick="markAsPaid(${request.tracking_id})" title="تأكيد الدفع">
                    <i class="fas fa-money-bill-wave"></i> تم الدفع
                </button>
            </td>
        `;
...
// تأكيد الدفع
function markAsPaid(trackingId) {
    const request = requestsData.find(r => r.tracking_id == trackingId);
    if (!request) {
        showMessage('لم يتم العثور على الطلب', 'error');
        return;
    ...
    const info = document.getElementById('paymentRequestInfo');
    info.innerHTML = `
        <div class="request-summary">
            ...
            <div class="detail-row">
                ...
                <span class="detail-value" style="color: #28a745; font-weight: bold;">${request.amount ? request.amount + ' ريال' : 'غير محدد'}</span>
            </div>
        </div>
    `;
...
async function confirmPayment() {
    ...
    try {
        const response = await fetch('../backend/api/finance/requests.php', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                ...
                notes: `تم الدفع${notes ? ' - ملاحظات: ' + notes : ''}`
            })
        });
        ...
        if (result.success) {
            ...
            showMessage(result.message || 'حدث خطأ أثناء تأكيد الدفع', 'error');
        }
    } catch (error) {
        console.error('خطأ في تأكيد الدفع:', error);
        showMessage('حدث خطأ أثناء تأكيد الدفع', 'error');
    }
}

// دالة مساعدة لتحويل طريقة الدفع إلى نص عربي
...
window.onclick = function(event) {
    const modals = ['viewRequestModal', 'markAsPaidModal'];
