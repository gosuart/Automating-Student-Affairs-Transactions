...
function setupStudentAffairsPage() {
    ...
    document.getElementById('userName').textContent = currentUser.name || 'موظف شؤون الطلاب';
...
async function loadRequests() {
    ...
    try {
        const response = await fetch('../backend/api/archive/requests.php?action=get_cards_data');
...
async function confirmApproveRequest() {
    ...
    try {
        const response = await fetch('../backend/api/archive/requests.php', {
            method: 'POST',
            headers: {
...
window.onclick = function(event) {
    const modals = ['viewRequestModal', 'approveRequestModal'];
